...
namespace LeoLuz.PlugAndPlayJoystick
{
    ...
    public class IJoystickInput
    {
        public ReactiveProperty<Vector2> screenInput;
    ...
    public class AnalogicKnob : ButtonBase
    {
        ...
        [ReadOnlyInPlayMode]
        [Inject]
        public IJoystickInput NormalizedAxis;
        ...
        public override void Start()
        {
            ...
            NormalizedAxis.released.Value = true;
        ...
        void Update()
        {
            ...
            lastFrameNormalizedAxis = NormalizedAxis.screenInput.Value;
            ...
            {
                if (NormalizedAxis.released.Value)
                {
                ...
                if (InsideAreaFingerId != -1)
                {
                    ...
                    if (NormalizedAxis.released.Value)
                    {
                        ...
                        {
                            NormalizedAxis.released.Value = false;
            ...
            Input.SetAxisMobile(HorizontalAxis, NormalizedAxis.screenInput.Value.x);
            Input.SetAxisMobile(VerticalAxis, NormalizedAxis.screenInput.Value.y);

            if (Mathf.Abs(lastFrameNormalizedAxis.x) < 0.2f && NormalizedAxis.screenInput.Value.x != 0f)
            {
                Input.PressButtonDownMobile(HorizontalAxis);
            }
            if (Mathf.Abs(lastFrameNormalizedAxis.y) < 0.2f && NormalizedAxis.screenInput.Value.y != 0f)
            {
        ...
        void TouchMove(Vector3 position)
        {
            ...
            NormalizedAxis.screenInput.Value = direction * normalizedDistance;
            transform.position = canvasInitialPoint + (Vector3)((StartPosition + (NormalizedAxis.screenInput.Value * _MaxDistance)) * ScreenUnitsToWorldUnitsConversionConstant);
        ...
        void TouchEnd()
        {
            NormalizedAxis.released.Value = true;
            InsideAreaFingerId = -1;
            NormalizedAxis.screenInput.Value = new Vector2(0f, 0f);
